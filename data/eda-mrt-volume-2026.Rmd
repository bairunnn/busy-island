...
# Important - divide by # of weekdays & weekends!

volume_raw <- volume_raw %>%
  mutate(across(
    c(TOTAL_TAP_IN_VOLUME, TOTAL_TAP_OUT_VOLUME),
    ~ if_else(DAY_TYPE == "WEEKDAY", . / 21, . / 10)
  ))
```
...
volume_processed <- volume_processed %>%
  mutate(
    ...
    PGLRT = if_else(if_any(c(PT_CODE_1, PT_CODE_2, PT_CODE_3), ~ str_detect(.x, "PW|PE|PTC")), 1, 0),
    SKLRT = if_else(if_any(c(PT_CODE_1, PT_CODE_2, PT_CODE_3), ~ str_detect(.x, "SW|SE|STC")), 1, 0),
...
summary_weekdays <- volume_weekdays %>%
  ...
  mutate(across(c(total_in, total_out, total_sum), round))
